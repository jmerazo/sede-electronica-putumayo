@extends('dashboard') 
<script>
function deleteEntitySetting(settingId) {
    if (!confirm('¿Seguro que deseas eliminar la configuración?')) return;

    fetch(`/dashboard/settings/${settingId}`, {
        method: "DELETE",
        headers: {
    ...
    .then(({ status, body }) => {
        if (status === 200) {
            alert(body.message || "Configuración eliminada con éxito.");
            location.reload();
        } else {
            console.error("Error al eliminar:", body);
            alert(body.message || "No se pudo eliminar la configuración.");
        }
    })
    .catch(error => {
    ...
    <div class="config-layout">
        ...
        <div class="entity-card-horizontal">
            <div class="entity-logo-section">
                @if($a->logo_path)
                    <img src="{{ asset('storage/' . $a->logo_path) }}" alt="Logo" class="entity-logo-xl">
                @else
