...
<div class="container-users">
    <div class="navbar">
        <div class="navbar-header-title">
            <img src="{{ asset('icon/permission-white.svg') }}" class="submenu-icon-area">
            <h2 class="navbar-title">Permisos</h2>
        </div>
        ...
        <div class="navbar-filters">
            ...
            <input type="text" id="search-input" class="search-box" placeholder="Buscar permisos...">
                ...
                @foreach ($permissions as $permission)
                    <tr>
                        ...
                        <td class="action-icons">
                            <a href="#" class="btn-icon" onclick="openEditModal({{ $permission->id }})">
                                <img src="{{ asset('icon/edit.svg') }}" alt="Editar">
                            </a>
                            <a href="#" class="btn-icon" onclick="deleteArea({{ $permission->id }})">
                                <img src="{{ asset('icon/destroy.svg') }}" alt="Eliminar">
                            </a>
                            <form id="delete-form-{{ $permission->id }}" action="{{ route('dashboard.permissions.destroy', $permission->id) }}" method="POST" style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>
                        </td>
...
<!-- Modal -->
<div id="modalCreatePermission" class="modal-overlay">
...
function openModal() {
    document.getElementById('modalCreatePermission').style.display = 'flex';
}

/* Modal */
function closeModal() {
    document.getElementById('modalCreatePermission').style.display = 'none';
}
...
document.getElementById("createAreaForm").addEventListener("submit", function (event) {
    ...
    fetch("{{ route('dashboard.permissions.store') }}", {
        method: "POST",
        body: formData,
        headers: {
    ...
    .catch(error => {
        ...
        alert("Hubo un problema al crear el permiso.");
    });
});

/* Update */
function openEditModal(areaId) {
    fetch(`/dashboard/permissions/${areaId}/edit`)
        .then(response => response.json())
        .then(data => {
            document.getElementById('edit_id').value = data.id;
            document.getElementById('edit_name').value = data.name;
            document.getElementById('modalEditPermission').style.display = 'flex';
        })
        .catch(error => console.error("Error al cargar datos:", error));
}

function closeEditModal() {
    document.getElementById('modalEditPermission').style.display = 'none';
}
...
document.getElementById("editAreaForm").addEventListener("submit", function (event) {
    ...
    fetch(`/dashboard/permissions/${areaId}`, {
        method: "POST",
        body: formData,
        headers: {
            "X-Requested-With": "XMLHttpRequest",
